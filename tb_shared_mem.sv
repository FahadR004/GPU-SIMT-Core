    ...
    initial 
        begin 
            ...
            for (i = 0; i < NUM_THREADS; i++) begin
                thread_mask[i]    = (i < 2) ? 1'b1 : 1'b0;  // only threads 0 and 1 active
                thread_write[i]   = 1'b0;
                th_bank_addr[i]   = 0;                        // both hit bank 0
                th_word_addr[i]   = i;                        // different rows → conflict
            end
            @(posedge clk); #1;
            $display("Conflict test — stall[0]=%b stall[1]=%b conflict_out[0]=%b", thread_stall[0], thread_stall[1], conflict_out[0]);
